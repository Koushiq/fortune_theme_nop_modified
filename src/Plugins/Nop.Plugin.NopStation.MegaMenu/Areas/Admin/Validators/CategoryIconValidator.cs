    ...
    public class CategoryIconValidator : BaseNopValidator<CategoryIconModel>
    {
        public CategoryIconValidator(ILocalizationService localizationService)
        {
            RuleFor(x => x.PictureId)
                .GreaterThan(0)
                .WithMessage(localizationService.GetResource("Admin.NopStation.MegaMenu.CategoryIcons.Fields.Picture.Required"));
            RuleFor(x => x.CategoryId)
                .GreaterThan(0)
                .WithMessage(localizationService.GetResource("Admin.NopStation.MegaMenu.CategoryIcons.Fields.Category.Required"));
        }
    }
}
